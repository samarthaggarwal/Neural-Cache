...
class initialize_dataset:
    ...
    def load_dataset(self, transform=False):
        ...
        if self.MNIST:
            ...
            size = 500
            train_dataset = torch.utils.data.random_split(train_dataset, [size, len(train_dataset)-size])[0]
            print(type(train_dataset), len(train_dataset))
            ...
            size = 50
            test_dataset = torch.utils.data.random_split(test_dataset, [size, len(test_dataset)-size])[0]
            print(type(test_dataset), len(test_dataset))
